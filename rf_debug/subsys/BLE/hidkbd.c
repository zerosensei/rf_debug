...
uint16 HidEmu_ProcessEvent(uint8 task_id, uint16 events) {
    ...
    if (events & BLE_SEND_DATA_EVT) {
        ...
        if (lwrb_get_full(&RF_SEND)) {
            lwrb_peek(&RF_SEND, 0, &report_id, 1);

            if (report_id == KEYNORMAL_ID) {
                lwrb_peek(&RF_SEND, 1, keyVal, 8);
