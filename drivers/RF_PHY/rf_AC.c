...
void AC_timesync( size_t len )
{
  ...
  {
    TMR0_TimerInit( SYSTOMS(sysclk) * TX_ONCETIME - AHEADTIME( sysclk ) - RECVTIME( len, sysclk ) - \
        RECVTIME( BLE_BUFF_MAX_LEN, sysclk ) - SYSTOUS(sysclk) * 150);
  }
...
{
#if(CONFIG_RF_MODE == 1)
//    if(((RTC_GetCycle32k()+0xA8C00000-rtctime)%(32*TX_ONCETIME) > (32*TX_ONCETIME-4)) && (worksta & RFTX))
//    {
//      while((RTC_GetCycle32k()+0xA8C00000-rtctime)%(32*TX_ONCETIME));
//      {
//          rtctime = RTC_GetCycle32k()+32*TX_ONCETIME;
